...
$("#Show_pSignIn").bind("click", signTab.login);
$("#Show_pSignUp").bind("click", signTab.reg);

//登陆注册弹窗
$("#signClose").bind("click", function (event) {
    $(".popwin-bg:not('.feedback-pop')").hide();
});
$("#signOpen").bind("click", function (event) {
    $(".popwin-bg:not('.feedback-pop')").show();
    signTab.login();
});

$("#registerOpen").bind("click", function (event) {
    $(".popwin-bg:not('.feedback-pop')").show();
    signTab.reg();
});
...
// 添加是否同意协议
$(function(){
    var objAgreement = $('#J_sign_argeement'), objBtn = $('#J_sign_submit');

    objAgreement.on('click', function(){
        var _checked = $(this).is(":checked");
        if(_checked) {
            objBtn.attr('disabled', false).removeClass('disbled-btn');
          } else {
            objBtn.attr('disabled', true).addClass('disbled-btn');
          }
    });

    objBtn.on('click', function(e){
        var _checked = objAgreement.is(":checked");
        if(!_checked){
            objAgreement.focus();
            $(this).attr('disabled', true).addClass('disbled-btn');
            e.preventDefault();
            return false;
        }        
    });
...
$(function () {
    ...
    $(".item").click(function () {
        ...
        var idx = $(this).index(),
            ...
            _speed =  Math.abs(400 - (400 / itemLen) * (itemLen - idx));
        ...
        $(".solution-nav-lv2").eq(idx).css({
            ...
            left: "0"
        }, {duration: _speed}).animate({
            width: "100%",
            padding: "0 138px"
        }, { easing: 'easeInQuart', duration: 400}); 
